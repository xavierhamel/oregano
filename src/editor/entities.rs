...
impl Entities {
    pub fn new() -> Self {
        let entities: Vec<Box<dyn entity::Entity>> = vec![
            ...
            Box::new(lumped::capacitor(Point::new(300.0, 200.0), "C0".to_string())),
            Box::new(source::voltage_ac(Point::new(100.0, 195.0), "Vin".to_string())),
            //Box::new(lumped::node(Point::new(200.0, 163.0), "n0".to_string())),
            //Box::new(lumped::node(Point::new(290.0, 203.0), "n1".to_string())),
            Box::new(lumped::ground(Point::new(170.0, 210.0), "gnd".to_string())),
            Box::new(probe::voltmeter(Point::new(300.0, 180.0), "P0".to_string())),
            
            Box::new(wire::Wire::new(Point::new(180.0, 210.0), Point::new(200.0, 200.0), wire::color())),
            Box::new(wire::Wire::new(Point::new(280.0, 210.0), Point::new(300.0, 210.0), wire::color())),
            Box::new(wire::Wire::new(Point::new(380.0, 210.0), Point::new(380.0, 170.0), wire::color())),
            Box::new(wire::Wire::new(Point::new(380.0, 170.0), Point::new(100.0, 170.0), wire::color())),
            Box::new(wire::Wire::new(Point::new(100.0, 170.0), Point::new(100.0, 210.0), wire::color())),
    ...
    pub fn select(&mut self, mouse: &mut mouse::Mouse) {
        ...
        for entity in &mut self.entities {
            ...
            && entity.connections_collide_with_point(mouse.scene_pos).is_none()
    ...
    pub fn add_floating_component(&mut self, component: &components::Components) {
        let mut new_component = component.generate(self.selected.len());
        new_component.set_is_selected(true);
        new_component.set_selected_offset(
            Point::new(
    ...
    pub fn add_component(&mut self) {
        if let Some(component) = &mut self.floating_component {
            ...
            self.entities.push(Box::new(component.clone()) as Box<dyn entity::Entity>);
    ...
    pub fn add_floating_wire(&mut self, origin: Point) {
        self.floating_wire = Some(wire::Wire::new(
            origin,
            origin,
            wire::color(),
        ));
    ...
    pub fn add_wire(&mut self, origin: Point) {
        if let Some(wire) = &self.floating_wire {
            self.entities.push(Box::new(wire.clone()) as Box<dyn entity::Entity>);
            ...
            if !entity.is_wire() {
                for (conn_idx, conn) in entity.connections().iter().enumerate() {
                    for other_entity in &self.entities {
                        if entity.origin() != other_entity.origin() || entity.shape() != other_entity.shape() {
                            if other_entity.is_wire() {
                                ...
                                for other_conn in other_entity.connections().iter() {
                                    if *other_conn + other_entity.origin() == *conn + entity.origin() {
                                        connections.push((idx, conn_idx));
                                    }
                                }
                            }
    ...
    pub fn update_selected_entity_properties(&mut self) {
        if self.selected.len() == 1 {
            ...
            for (&key, current_property) in entity.properties().iter() {
                if let Ok(property) = entity::Property::from_inputs(&format!("property__{}", key)) {
