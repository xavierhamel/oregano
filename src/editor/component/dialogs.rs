use crate::editor::{component::components, entity::Entity, property};
    ...
    for (title, components) in sections.iter() {
        ...
        let buttons = components
            ...
            .map(|comp| {
                ...
                let canvas = dom::canvas::create(dom::attributes! {
                    "width" => &width_string[..],
                ...
                let button = dom::create_element(
                    ...
                    vec![canvas, dom::form::label::create(component.short_name)],
                );
                {
...
pub fn load_properties_dialog(component: &Box<dyn Entity>) {
    ...
    let mut properties = component
        .properties()
        .iter()
        .map(|(key, property)| (property, key.clone(), property::metadata_en(&key)))
        .collect::<Vec<(&property::Property, String, (&str, &str, usize))>>();
    properties.sort_by_key(|k| k.2 .2);

    for (property, key, (title, desc, _)) in properties.iter() {
        let group = match property {
            property::Property::Text(name, is_visible) => dom::form::group(vec![
            ...
            property::Property::Num(number, is_visible) => dom::form::group(vec![
            ...
            property::Property::Unit(value, prefix, unit, is_visible) => {
                ...
                dom::form::group(vec![
                    ...
                    dom::form::text_input::create(
                        &format!("property__{}", key),
                        &value.to_string(),
                    ),
            ...
            property::Property::InternalStr(_) | property::Property::InternalF64(_) => {
                continue;
            }
        };
        dom::append_children(
